...
int main()
{
   ...
   int shmid, numero = 0,factorial = 1, a;
   ...
   numero = shared_stuff->some_text[0];
                  // ciclo hasta que el proceso lea del area de memoria compartida el entero "-1"
   while (running) {
      if (shared_stuff->written_by_you) {
            numero = shared_stuff->some_text[0];

	 for(a = numero; a > 1; a--){//determinar factorial
       		factorial = factorial * a;
  	 }

         printf("El factorial de %d es: %d\n",shared_stuff->some_text[0],factorial);
         factorial = 1;
         sleep( rand() % 4 );         /* espera por un tiempo aleatorio para que el proces productor espere */
         shared_stuff->written_by_you = 0;
         if (shared_stuff->some_text[0] == -1) {
	    printf("Bye\n");
            running = 0;
         }
      }
